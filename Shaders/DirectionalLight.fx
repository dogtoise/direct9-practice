	...
	) : COLOR0
{
	...
	float3 reflect = normalize(2 * dot((float3)normalMap, LightDir) * (float3)normalMap - LightDir);


	// Compute diffuse 
	float4 diffuseColor = Diffuse * DiffuseIntensity * max(0, dot((float3)normalMap, LightDir));
